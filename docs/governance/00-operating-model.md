...
on a real project, so you inherit a model that is grounded in practice — not opinion 
handed down, and not someone else's archaeology to dig through.
...
at a deliberate version bump — not silently copied.
...
- [ ] Start from a **de-projected Part 1**; strip every prior-project specific.
...
> track record is still limited — what feels universal here is not yet *proven* universal.
> The first time a fresh project starts on it is the real test, and that test should
> itself become a lesson.
